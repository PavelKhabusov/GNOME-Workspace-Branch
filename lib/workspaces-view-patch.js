...
class GridWorkspacesView extends WSV.WorkspacesView {
    ...
    vfunc_allocate(box) {
        ...
        // Промежуточные allocation во время анимации перехода в/из overview
        // приходят с вырожденным box (width/height ≈ 0). Тогда cellW/cellH
        // ниже становятся отрицательными → воркспейсы и WindowPreview внутри
        // получают отрицательный размер → Clutter сыпет assertion'ами и всё
        // мерцает. Пропускаем такие кадры на нативную раскладку.
        if (width <= 1 || height <= 1) {
            super.vfunc_allocate(box);
            return;
        }
        ...
        const spacingAll = 16;
        // Клампим в неотрицательное: при узком box и большом cols/rows деление
        // могло дать отрицательный размер ячейки.
        const cellW = Math.max(1, (width  - (cols + 1) * spacingAll) / cols);
        const cellH = Math.max(1, (height - (rows + 1) * spacingAll) / rows);
        ...
        for (let i = 0; i < this._workspaces.length; i++) {
            ...
            const w = Math.max(1, singleW + (allW - singleW) * allWeight);
            const h = Math.max(1, singleH + (allH - singleH) * allWeight);
