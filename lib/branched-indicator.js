...
export class BranchedIndicator {
    ...
    install() {
        ...
        if (this._settings) {
            ...
            // Drum rotation мутирует appendages без add/remove ws-сигналов —
            // ловим перерендер через settings.
            this._appendagesId = this._settings.connect(
                'changed::appendages', () => this._rebuild());
        }
    ...
    uninstall() {
        ...
        if (this._appendagesId && this._settings) {
            this._settings.disconnect(this._appendagesId);
            this._appendagesId = 0;
        }
