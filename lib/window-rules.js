...
function countAppWindows(desktopId, currentWin) {
    const tracker = Shell.WindowTracker.get_default();
    if (!tracker) return 0;
    ...
    for (let i = 0; i < wm.n_workspaces; i++) {
        ...
        for (const w of ws.list_windows()) {
            if (w === currentWin) continue;
            if (w.skip_taskbar || w.is_on_all_workspaces()) continue;
            const app = tracker.get_window_app(w);
            if (app && app.get_id() === desktopId) count++;
        }
    }
    return count;
}
...
function tryApply(win) {
    ...
    for (const rule of _rules) {
        ...
        if (rule.stack && rule.match?.desktop_id) {
            const offset = countAppWindows(rule.match.desktop_id, win);
            if (offset > 0) {
                target = {
                    ...target,
                    layer: (target.layer ?? 0) + offset,
                };
            }
        }
