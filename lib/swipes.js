...
const PANEL_SMOOTH_THRESHOLD = 8; // px суммарной dy для срабатывания смены ws
...
export class Swipes {
    ...
    enable() {
        ...
        // перехватывает UPDATE-события раньше нас. 4-finger overview show/hide
        // делаем мы сами.
        ...
        if (Main.wm && !this._origHandleWsScroll) {
            ...
            Main.wm.handleWorkspaceScroll = function (event) {
                ...
                // Тачпадные scroll-события на Wayland приходят как SMOOTH
                // с дельтой. Без аккумуляции наш патч их пропускал, поэтому
                // двухпальцевый скролл по панели ничего не делал.
                if (dir === Clutter.ScrollDirection.SMOOTH) {
                    const [, dy] = event.get_scroll_delta();
                    if (dy === 0) {
                        self._panelSmoothAccum = 0;
                        return Clutter.EVENT_STOP;
                    }
                    self._panelSmoothAccum = (self._panelSmoothAccum || 0) + dy;
                    if (Math.abs(self._panelSmoothAccum) < PANEL_SMOOTH_THRESHOLD)
                        return Clutter.EVENT_STOP;
                    if (self._panelSmoothAccum < 0) nav.switchUp();
                    else                            nav.switchDown();
                    self._panelSmoothAccum = 0;
