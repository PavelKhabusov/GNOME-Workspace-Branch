...
function tryLaunch() {
    ...
    for (const rule of allRules) {
        ...
        // Маркер «уже стартовали в этой сессии» уважаем, только пока сам app
        // действительно жив. $XDG_RUNTIME_DIR изредка переживает logout (если
        // systemd-сессия не закрылась), и тогда маркер ложно блокирует старт
        // после реального reboot/relogin.
        if (launched.has(did)) {
